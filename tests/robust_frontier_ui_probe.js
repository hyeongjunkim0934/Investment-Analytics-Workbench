...
const initial=chart(),zoomRange=initial.opts.scales.x.range.slice();
click('표');
...
assert.equal(+input().value,1);assert.equal(button('강건 구간').getAttribute('aria-pressed'),'true');
assert(width(initial)>200);assertContained(initial);
assert.equal(draw(initial).filter(x=>x[0]==='gradient').length,1);assert.equal(draw(initial).filter(x=>x[0]==='stop').length,3);
assert(!visibleLabels(initial).includes('BM 60/40'));
const zoomWidth=width(initial);
click('전체');const all=chart(),allRange=all.opts.scales.x.range.slice();
assert.equal(button('전체').getAttribute('aria-pressed'),'true');assert(allRange[1]>zoomRange[1]);assert(visibleLabels(all).includes('BM 60/40'));
...
change(0);assert(chart().data[1].every((x,i)=>x===chart().data[2][i]));assert.equal(draw(chart()).filter(x=>x[0]==='gradient').length,0);
assert.equal(button('전체').getAttribute('aria-pressed'),'true');
const zeroKLegend=card().querySelector('.port-frontier-key').textContent;
