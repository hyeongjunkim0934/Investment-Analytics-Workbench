...
| `tests/domshim.js` `tests/dashboard_probe.js` | `dashboard/app.js` 를 **node 안에서 실제로 실행**시키는 최소 DOM 셰이드와 측정 하네스. npm 의존성 0(node 표준 라이브러리만). 소스 문자열만 보는 테스트가 "이름은 남기고 동작만 뒤집는" 회귀를 못 잡아서 만든 것이다 — 이 하네스를 만들 때 쓴 뮤테이션 18건은 18/18을 잡는다. **다만 이것을 전면 커버리지로 읽지 말 것**: 독립적으로 만든 뮤테이션 32건으로 다시 재면 24/32였다. 그때 **안 잡히던 여덟 자리는 2026-08-17 에 전부 테스트를 붙여 닫았다**(각 뮤테이션을 실제로 적용해 빨간불을 확인함): ① `stampLatest` 다계열 최신 인덱스(`multiSeriesStaggered` 프로브) ② `baseAxes` refmt 무조건 적용(`uniqueLabels`) ③ 오버레이 `body.style.overflow` 복구(`bodyOverflow`) ④ `aria-current`(`navCurrent` 프로브) ⑤ 뒤 규칙의 `:focus-visible` outline 무력화(시트 전수 검사 — **`.vz` 의 `outline:none` 은 결함이 아니라** 같은 컴포넌트의 라벨 색·밑줄이 대체 표시라, 판정은 「대체 표시 없는 제거」만 잡는다) ⑥ `stampLatest` 중복 가드(`dupCount`) ⑦ 본문 바로가기 링크 마크업 검사 ⑧ 모션 축소 블록 **뒤**의 `scroll-behavior:smooth` 재선언 검사. 같은 부류의 새 공백은 계속 생길 수 있다 — 손댈 때 테스트를 먼저 붙일 것. app.js가 새 DOM API를 쓰면 셰이드가 먼저 터지므로 그때 채워 넣으면 된다(그렇게 `after()`·`play()`/`pause()`·`visibilityState` 를 채웠다). **장면 자동 순환 검사도 여기 있다** — `setInterval` 을 기록만 하도록 바꿔 15초를 기다리지 않고 틱 본문을 직접 돌린다(자작 뮤테이션 20/20, 같은 이유로 과대평가로 읽을 것). **환헤지 2차 패스(141개)에도 같은 성격의 공백 10곳이 실측돼 있었다** — 부호·단위 자리 넷은 이제 전부 닫혔다(2026-08-17 뮤테이션 재확인): `carryTxt` 의 ± 와 받는/내는 분류는 E-6 테스트가 이미 잡고 있었고, MTM 연율화 `√12` 는 `mtmVol` 검사를 새로 붙였으며, `#fx` 의 정적 부호 문장은 §7.9 재편 때 문장 자체가 사라져 소멸했다(2026-08-17 grep 재확인 — 정적 부호 문장은 index.html 에 더 없다). **남은 공백은 산문·표기 6곳**(⑤~⑩) — 목록은 `docs/HANDOVER.md` §5.3.1, 상태 갱신은 그 절 끝의 2026-08-17 추기 |
| `tests/test_robust_frontier.py` | 강건 포트폴리오 JS 엔진을 직접 실행해 독립 KKT·2자산 격자·표본수 스케일·κ=0·공분산 경계조건 검증 (§7.24) |
| `tests/test_optimistic_frontier.py` | Optimistic 고정 위험 단면과 비지배 경계를 독립 2/3자산 격자·이차식으로 검증. `portRobustModel.atRisk`는 전체 simplex 면을 열거해 최대 평균을 구하고, `portFrontiers.optimistic`은 상방 불확실성을 적용한다. 표시명은 Conservative/Optimistic이며 기존 `robust` 내부·저장 키는 호환 유지. |
...
# 테스트 (합성 픽스처 — ../Data 없이 돈다, 약 4분). 전체 수는 --collect-only로 확인.
...
#     (simPanel·lambdaControl — cmaTv 는 §7.15 카드 제거와 함께 삭제)가 대부분이다. 프로브를 추가할 때 반복 횟수를
...
- **15는 코드가 정한 수**(아래 JSON 계약)라 달라지면 그 자체가 버그다. 반대로 `N`·`M`·경고 건수는
  ...
  **495 시리즈 / 7 파일 / 경고 13건**(duplicate column 11건 = 진짜 중복 skip 1건(`미국_IRS_5y1y`)
  + 아래 헤더 행으로 **분리** 10건, + 파일 간 병합 고지 1건(두 BM 파일 10계열), + port 창 미충족
  고지 1건 — 원화유동성 표본이 10년을 채우면 저절로 사라진다)이며, 이 기준선의 정본은 Data 저장소
  쪽(`../Data/CLAUDE.md`)이다. 2026-09-09 익스포트 교체(bb 파일이 `data_bb_대체투자 (2).xlsx`
  하나로 합쳐지고 12M 선행 EPS·지수 8열이 붙음 — §7.22) 이전 체크아웃이면 483 / 8 / 경고 14건,
  2026-08-21 교체 전(구 `data_bb.xlsx`)이면 481 / 8 / 16건으로 나온다. 급감이나 경고 성격 변화만
  신호로 볼 것.
...
- `meta.json` 은 빌드 시각·최종 관측일·시리즈 수·파일별 파싱 결과·경고 목록을 담는다.
  2026-09-09 사용자 요청으로 푸터의 데이터 설명·빌드 내역·경고 펼침은 제거했다.
  헤더(`#meta-line`)에는 기준일·빌드·시리즈 수를 표시하고, 경고는 `meta.json`과
  콘솔(`pipeline warnings:`)에 유지한다. 화면별 계산 불가·비중 오류 표시는 유지한다.
...
- 같은 시트에 같은 Notation이 두 번 나오면 **첫 컬럼이 그 라벨을 갖고**, 뒤 컬럼은 Notation 행
  **아래**의 헤더 행(블룸버그 티커·필드 행)에서 첫 컬럼과 다른 셀을 접미사로 붙여 `라벨 (접미사)`
  키로 살린다(`process.dup_suffix`, 2026-09-09 §7.22 — 예: `bb:KOSPI` = 12M 선행 EPS,
  `bb:KOSPI (PX_LAST)` = 지수. 티커만 다르면 티커, 둘 다 다르면 `티커 · 필드`). 아래 헤더까지 전부
  같으면 진짜 중복이라 경고 후 버린다(현재 1건 — `미국_IRS_5y1y`). Notation **위** 행(Start/End
  Date)은 보지 않는다 — 진짜 중복도 그 행은 다르다. 어느 쪽이든 경고가 남고 접미사는 파일이 준
  문자열 그대로다 — 코드가 이름을 짓지 않는다.
  **분리를 "해결"로 읽지 말 것** — 2026-08-21 실측에서 버려지던 컬럼 7개 중 **6개는 값이 서로 다른
  별개 시리즈**였고(같은 라벨을 공유할 뿐), 분리는 그 데이터를 잃지 않게 할 뿐 라벨 충돌 자체는
  그대로다(어느 컬럼이 맨 라벨을 갖는지는 여전히 **컬럼 순서**가 정한다). 어떤 라벨이 어떤 상품이었는지는
  비공개 `../Data` 의 CLAUDE.md 에 실측 표로 적어 두었다(벤더 값이라 여기에는 옮기지 않는다). 그중
  **하나는 첫 컬럼 자체가 오라벨**이라(`bb:호주_BEI_10y` 가 나르는 값은 5년물, 진짜 10년물은 분리
  키) `catalog.json` 에 나가는 키 이름이 실제 내용과 다르다 — 그 키를 읽는 코드가 없어 게시된
  **값**은 영향받지 않지만, **공개 메타데이터가 틀린 상태**다. 고치는 자리는 이 저장소가 아니라
  **벤더 익스포트 템플릿의 Notation 라벨**이며, 라벨을 고치면 분리 키가 사라지므로 그 키를 후보
  체인으로 읽는 곳(`risk.PER_SOURCES`, `process.OVERVIEW_CARDS`)을 함께 고친다.
  같은 키가 여러 파일에 있으면
...
- **밸류에이션·이익 리비전(잠재 위험)은 12M 선행 EPS 로 산다(§7.22, 2026-09-09 업로드).**
  재료 키는 `risk.PER_SOURCES` 후보 체인 한 곳이다 — 시장(KOSPI·S&P 500)마다 EPS 키와 가격지수
  키를 **순서대로** 찾아 처음 있는 것을 쓰고, 어느 키를 썼는지 요인 `note` 에 게시한다. 선행 PER =
  가격지수 ÷ 12M 선행 EPS(`hi`), 리비전 = EPS 의 **달력 3개월 전** 대비 변화율(`lo` — PER 에서
  역산하지 않는다). **ACWI 선행 EPS 는 익스포트에 없어 글로벌 축은 S&P 500** 이며 note 가 그
  사실을 적는다(NIKKEI·EUROSTOXX50 EPS 도 파싱되지만 요인에는 넣지 않았다 — 지역 구성 변경은
  사용자 합의 뒤). 두 시장 중 하나라도 재료가 없거나 짝 검증(`PER_SANE` — 지수÷EPS 전 이력
  중앙값, 모형 모수가 아니라 무결성 가드)에 걸리면 **요인 전체를 사유와 함께 보류**한다(한 시장만으로
  살리면 합성의 지역 구성이 조용히 바뀐다). `limits` 의 대기 문장은 보류일 때만 붙는다. 개요
  KOSPI·S&P 500 카드(`OVERVIEW_CARDS` 첫 원소가 튜플 = 후보 체인, `get_first`)와 `kospi10`
  대조축도 같은 가격지수 후보 튜플을 쓰며 카드 `key`·`kospi10.src` 에 실제 키가 실린다.
...
- **자산배분 화면의 위험 원천은 스위치다 — CMA(기관 벤치마크)가 기본, 벤더 프록시는 대조용.**
  ...
  핵심 성질, 전부 프로브 `cmaLayer` 가 실행으로 확인한다: ① **BM 계열마다 자체 헤지
  스탠스 h₀ 가 다르다** — 기관이 자산군별 실제 정책 그대로 지수를 산출하기 때문이다
  (2026-08-17 사용자 확인 "시가해외주식 BM 은 언헤지, 주식은 오픈이 기본"). 정본은
  `app.js` 의 **`CMA_BM_H0`**(해외채권 1 = 헤지 / 해외주식 0 = 미헤지)이고 노출은
  **BM + (h₀ − h)·`_fx`** 다 — h = h₀ 면 보정이 0. Xe 붕괴는 그대로 성립한다.
  **단순상관으로 h₀ 를 판정하지 말 것 — 그렇게 했다가 틀렸다(§7.7.19).** ACWI 자체가
  달러원과 음의 상관이라 미헤지 원화환산 해외주식도 단순상관이 0 근처로 **상쇄**되는데,
  그 0 을 「환노출이 없다」로 읽어 전 계열을 헤지 기준으로 처리했고 해외주식 환노출이
  `w주식` 만큼 이중계상됐다(헤지 축이 한 칸 밀려 화면의 「완전헤지」가 실제로는
  「완전오픈」). 판정은 다변량 회귀라야 한다: 해외주식 β(달러원) **+1.000**(SE 0.027,
  R² 97.9%) / 해외채권 **−0.048**(SE 0.024). 방증 — 게시 공분산에서
  corr(시가 해외주식 − `_fx`, `_fx`) = −0.646 으로 환을 벗기면 다른 원화 계열
  (해외채권 −0.589·국내채권 −0.612)과 같은 자리에 들어간다.
  **총 환노출 ≠ Xe** — 매핑 대체투자가 시가 해외주식(h₀=0)을 팩터로 써서 `_fx` 열이
  0 인데도 슬리브당 `we/100` 만큼 환을 진다. **Xe 는 최적화가 고르는 레버의 축**이라
  이 몫을 넣지 않고(§7.7.20 — 넣으면 최적 헤지쌍이 사용자가 정한 대체투자 헤지비율을
  덮어쓴다), 엔진 `fxLoadW()` 가 내재분까지 세어 화면이 **Xe 와 구분해서** 적는다.
  실브라우저 실측(2026-08-19, 기본 배분·기본 헤지 90/90): Xe **2.70%** · 대체투자가
  지는 환 **8.00%p**(대체 16% × 매핑 we 0.5) → `h_alt` 0% 면 총 10.70%(Xe 의 3.96배),
  코드 기본값(자리표시자)에서는 총 3.50%. **이 수는 `h_alt` 를 따라 움직인다** —
  고정 배수로 적지 말 것.
  ...
  **매핑 대체투자의 환은 `st.h_alt` 가 건다(§7.7.20 — 2026-08-19 사용자 지시
  "대체투자는 유동적으로 환헤지 하는중").** 팩터인 시가 해외주식이 h₀=0 이라 `we`
  만큼 환이 딸려 오고, 그중 `h_alt` 를 헤지해 `_fx` 에서 **뺀다**(더하면 헤지가
  노출을 늘리는 반대 모형이다) → 순 환노출 `we·(1 − h_alt)`. **두 분류에 한 비율**을
  걸지만 매핑이 채권 100%(we=0)면 걸 환이 없어 0 이라, 분류별 매핑이 달라도 한
  비율로 맞게 작동한다(슬라이더를 분류마다 두지 않은 근거). **Xe 에는 넣지 말 것** —
  이쪽은 최적화가 고르는 레버가 아니라 **모형 입력**이라(사용자 선택 — μ·σ·λ 와 같은
  칸, 즉시 저장·최적 ▼ 없음) 합치면 `allocJointOpt` 의 위험최소 Xe 가 사용자가 정한
  값을 조용히 덮어쓴다. 화면 정본은 `altHedgeBox()`(래퍼에 `.sim-alt-hedge` 를 더
  다는 이유는 래퍼만 세는 검사가 헤지 레버로 잘못 세지 않게 하려는 것 — §7.7.15 의
  `.sim8-row` 한정과 같은 함정). **디폴트 90 은 자리표시자이지 기관 수치가 아니다.**
  **헤지 효과의 부호를 상수로 적지 말 것** — 미헤지 BM 을 팩터로 쓰므로 자연헤지가
  그대로 옮겨 와 헤지가 위험을 **올릴** 수 있고, 프로브도 방향을 못박지 않고
  「직접 해외주식과 같은 방향인가」로 잰다(`altHedgeDirectionMatchesEquity`).
  ...
  **권장값을 코드에 박지 말 것** — λ 는 관측되지 않는 선호 모수라 "표준 숫자"가 없다
  ...
  즉시 저장을 실행으로 확인한다.
  **두 번째 앵커 = 손실 한도(2026-09-08, HANDOVER §7.20)** — `allocLambdaForLoss(E, H, α)`:
  "1년에 H% 넘는 손실은 α% 이내" 에서 λ 를 역산한다(Das·Markowitz·Scheid·Statman 2010
  심리계정, 정규 근사 `q(λ)=μ(λ)+Φ⁻¹(α)σ(λ) ≥ H`). q(λ) 는 λ 를 따라 **단봉**이라 해가
  0·1·2개다 — 불가(`feasible:false` + 달성 가능 최대 H)·느슨(`bounded:"low"`)·두 교점.
  **내재 λ = 공격적 교점**(한도 아래서 기대수익 최대 — 프레임의 선택)이고 보수적 교점도
  적는다. H·α 는 `st.loss_h`/`st.loss_a`(즉시 저장) 이며 **기본값이 없다** — 한도는 기관의
  결정이지 코드가 정할 수가 아니다(빈 칸이면 역산하지 않고 그 사실만 적는다). 「λ 에 적용」이
  `mvo_lambda` 를 바꾼다. 4×4 역산 행렬표(H −5…−20 × α 1…10%)는 같은 프론티어 격자를
  재사용한다. 프로브 `lossLambda`. **국면확률·리스크 점수를 λ 에 자동으로 곱하지 말 것** —
  같은 날 사용자가 가져온 3층 제안 중 그 층은 §5.1 실험이 이미 기각했다(경기순응).
  **시변·창 민감도 카드는 §7.15(2026-08-31 사용자 지시)로 제거됐다** — 효율적 투자선·
  이행 경로·포트폴리오 특성·자산군 표·출처 상자·방법론과 함께 표시층만 걷어냈다.
  `bm.py` 의 `cma.tv`(길이별 롤링 공분산) 게시는 **유지**된다(파이프라인 무변경 —
  재도입 여지·게이트 불변). 되살릴 때는 해당 커밋 revert 가 가장 빠르다(HANDOVER §7.15).
- **헤지 레버의 자유도는 실질 1개다 — 「최적 헤지비율 한 점」을 적지 말 것.**
  ...
  **CMA 층에서는 `_fx` 열 계수가 `Xe` 와 다르다 — 그런데 문제가 되지 않는다(§7.7.19).**
  계열별 h₀ 때문에 열 계수는 `Σwᵢ(h₀ᵢ−hᵢ)` 인 반면 `xeOf` 는 `Σwᵢ(1−hᵢ)` 를 돌려주고,
  둘의 차이는 `w주식` 이다. 그러나 그 차이가 **(hb,he) 와 무관한 상수 평행이동**이라
  등위집합이 그대로 보존된다 — 같은 Xe 를 만드는 쌍의 σ 는 여전히 정확히 같고(실측
  산포 0.000e+00), Xe 범위도 `[0, wF]` 그대로이며(실측 [0.000%, 27.000%]), 왕복도
  성립한다. `xeQuadW` 가 **로딩 산식을 재유도하지 않고 σ 를 세 번 재서** 2차식을
  맞추는 설계가 정확히 이것을 막아 준다(2차 적합 잔차 1.2e−14). **이 세 불변식은
  프로브 `isoXePairsGiveSameSigma`/`xeStaysInStructuralRange`/`fxLoadCountsDirectEquity`
  가 고정한다** — 감사에서 「좌표가 밀렸을 것」이라는 가설이 제기돼 실측으로 반증한
  자리이므로, Xe 정의를 손대려는 변경은 이 셋을 먼저 볼 것.
...
- **자산배분 화면의 물리적 최상단은 §7.14 포트폴리오 구성 패널이고(2026-08-21 사용자
  지시 「제일 상단」), 그 아래 첫 자리가 시뮬레이터다** (2026-08-11 사용자 지시 —
  2026-08-05 「요약 먼저」 동선을 대체). `#alloc-sim-panel` + `renderAlloc` 안의 시뮬레이터 블록이
  ...
  **통화별 환헤지 분해**(`allocCcyHedgeRows` + 접이식 표)는 슬리브 헤지비율을 통화
  ...
  구 배분 칸은 이 패널이 같은 id(`sim-mix-*`)로 승계했다. 그 아래는
  「요약 → 참고치·레버」까지다. 설정·저장·제약조건 입구는 시뮬레이터 위에 있다(설명 구역 — 특성·자산군 표·투자선·이행
  경로·시변·출처 상자·방법론 — 은 §7.15 로 제거):
- **자산배분 화면의 나머지 동선은 「요약(답) → 조작(콘솔)」 순이다** (2026-08-05 사용자
  확인 — 담당자의 메인 용례는 ① 최적 배분·헤지 조회 ② 자유 조정 시뮬레이션 둘이다.
  설명 카드 구역은 2026-08-31 §7.15 로 제거 — 주석은 키워드만, 경고·사유·⚠ 는 유지).
  2026-09-09 §7.21: 카드·목차·버튼명을 축약하고 중복 안내를 제거했다. 대체투자 표시명은
  `allocShortK`를 쓰되 내부 키·입력 식별자는 유지한다. 단위·출처·표본·저장 상태·보류 사유는
  축약 후에도 남긴다. UI 문구를 바꾸면 `dashboard_probe.js`의 해당 선택자도 함께 갱신한다.
  2026-09-09 추가 간소화: 상위 체계 2개만 유지하고 스크롤 목차 5개와 중복 헤드라인·체계 설명을 삭제했다.
  기관 설정은 `allocSelect` 선택창 4개와 합계 고정 체크박스로 전환했다. 기간은 `allocPeriodControl`로 통일하며 CMA·프록시 모두 지원한다.
  기관 본문의 설명 펼침 9곳은 삭제했고, 부분해 구분은 짧은 문장으로 남긴다. 엔진·초안·저장 규약은 유지한다.
...
- **자산배분 상위 탭은 포트폴리오·리스크연계 두 개다(2026-09-10).** 기관배분/헤지 탭과
  해당 탭으로 가던 배분 설정 버튼은 제거했다. `ALLOC_RETIRED_PANELS`는 리스크 계산의
  공통 엔진·저장 호환을 위해 DOM을 유지하지만 항상 숨긴다. 기존 상세 해시는 리스크연계로 복귀한다.
- **포트폴리오 개선여지(2026-09-13)**: 기존 `자산 공백` 표시를 `개선여지`로 바꿨다.
  `renderPortImprovement`는 벤치마크 비교 바로 아래에 신규 자산 조건표를 그린다.
  기준점은 기존 일반 경계선 실제 비중이며 기대수익 하한=기준 기대수익, 기본 신규 비중10%·
  변동성=기준의1배·기준 포트폴리오와 상관0이다. 기존 비중을 비례축소한 혼합의 위험이
  실제 줄어드는지 확인한다. 충분조건이며 유일한 개선 조합·실재 자산 전망·최적 편입비중은 아니다.
  자산별 상관은 기준 포트폴리오 공분산에서 유도하며 모두 같은 상관을 강제하지 않는다.
  가정은 명시적으로 적용할 때만 별도 저장하고 CMA·경계선·리스크 한도 입력은 바꾸지 않는다.
  `portRangeSigma`의 저장값 부재/오류 기본값은 **0.25**다. 유효한 기존 저장값과 0.25 증감 간격은 유지한다.
- **리스크연계 = `renderAllocRiskProc`**, 별도 workspace에서 현재·잠재를 동시에 표시한다.
  `risk.json.layers.*.hist_alloc`은 전체 W-FRI 주간 점수와 실제 기준일의 최신 점수다.
  기준일 이후 금요일 라벨은 제외하며 마지막 점은 리스크 헤더의 날짜·점수와 일치한다.
  기존 `hist`·`hist_m`은 다른 소비자를 위해 유지한다. IC 가중치 재학습 4주는 변경하지 않는다.
  λ는 원점수(0~100)×배율이며 기본 ×1, 선택 ×0.1(`rp_scale`)을 지원한다. ×0.1은 위험선호
  변경이지 단위 보정이 아니다. 기존 로그/선형 변환·기관 λ 앵커는 무시한다. `portModelInputs`로 현재 포트폴리오
  6자산의 μ·σ·상관 입력과 선택 표본을 읽는다. μ·Σ 고정 시나리오이며 역사적 백테스트가 아니다.
  일반 효율적 경계선과 같은 공매도 금지·합계 100% 조건이다. 그룹 입력은 현재 비중 초기화용이며
  최적화 제약이 아니다. 자산별 투자한도는 `rp_bounds`에 %로 저장해 리스크 최적화에만 적용한다.
  무제약은 자산별 한도를 해제하고 공매도 금지·합계100%를 유지한다. 범위 그림은 한도 및
  현재/잠재 × 제약/무제약의 최신 비중 4점을 표시한다. `rp_range`·`rp_scale`·`rp_mode`는 즉시 저장,
  한도는 명시적으로 적용할 때 저장한다. 미적용 초안과 펼침 상태를 재렌더에 보존한다.
  관측 사이 비중은 직전 해를 유지하는 계단형 누적 면적이며 점수×배율=λ 및 호버 λ를 표시한다.
  주간 경로 전용 `allocRiskOptimize` active-set QP는 원래 목적함수의 KKT·제약을 검증한
  해만 반환하며 기존 기관 solver를 변경하지 않는다. 입력과 λ가 같은 해만 캐시한다.
  기본 3Y, 1Y/5Y/전체 선택. 날짜 간격은 실제 timestamp, 수치는 표/CSV와 차트 내부 툴팁에
  제공한다. CSV는 선택 기간의 원시 정밀도를 유지한다. 포트폴리오 부재·잘못된 이력·제약 불충족은
  보류하며 월별로 몰래 대체하지 않는다. 그래프는 포트폴리오의 카드·팔레트·격자 스타일을 공유한다.
- **수익률 추정 화면은 §7.17(2026-09-07 사용자 지시)로 삭제됐다 — 되살리지 말 것.**
  지운 것은 **표시층뿐**이다: 탭·섹션·`estEngine`/`estScenario`/`renderEstimate`/
...
- **마을 안내판 = `paintVillageNotes`·`villageNotesModel` 이 정본이다(§7.18, 2026-09-07
  사용자 지시 "마을은 그냥 마을만" + "건물 옆 반투명 네모칸에 글씨가 움직이며").**
  **다섯 상자(§7.23, 2026-09-09 재구성)**: 종탑=위험·국면(그대로), 곳간=최근 변화 — 주식,
  저잣거리=채권·금리, 교역소=외환, 여관=크레딧·원자재(브리핑 대신 「일반 변화」 — 사용자
  "특이사항 말고 일반적인 최근 변화"). 자산배분·헤지 안내판은 걷었다. 붙는 건물·위/아래·`dx` 는
  `VILLAGE_NOTES` 가 정한다(저잣거리·교역소 아래, 저잣거리 `dx` +8 = 서고 라벨 회피 — 실브라우저
  겹침 측정으로 고른 값). **새 숫자가 없다** — 점수·등급은 risk.json, 최근 변화 4구역은
  **개요 카드(overview.json `cards`, `group` = equity/rate/fx/other)의 값·변화(직전/1일·1개월·
  YTD·1년)를 그대로** 옮기며 자릿수·단위는 개요 KPI 와 같은 `deltaNum` 한 벌이다(프로브
  `villageNotes` 가 개요 화면 문자열과 대조한다. 따로 계산하지 말 것). 화살표 대신 부호 —
  참고 표시 전용이라 동사(늘림/줄임/매수/매도)·▲▼ 금지, 정규식으로 잡는다. 페이로드가 없으면
  그 상자만 사유를 적는다. 계산은 `renderAll` 뒤에만 다시 한다(`VILLAGE_NOTE_CACHE`) — 15초
  장면 순환의 `renderVillage` 는 캐시를 다시 붙일 뿐이다(배분 저장은 이제 안내판과 무관).
...
- **두루마리 배너 = `mountVillageBanner`·`villageBannerSvg`·`VILLAGE_BANNER`(§7.19, 2026-09-07).**
  ...
  **제목은 그림에 굽지 않는다** — `villageBannerSvg()` 가 두루마리 면 위에 SVG 텍스트로 얹고,
  ...
  양피지의 **연결 성분**으로 잡은 필기면의 열별 중심을 2차 적합해 캡높이 절반만큼 내린
  경로이고(기울기 11.2°·처짐 5px), `vb`(키잉본 캔버스 721×568)와 **한 몸**이라 그림을 갈면
...
- **정의되지 않은 CSS 이름은 조용히 무효가 된다 — 회귀 테스트가 있다.**
  ...
  `.btn` 이 아니라 `.btn-ghost`/`.btn-primary` 다. CSS 는 오타에 오류를 내지 않고 DOM
  셰이드도 스타일을 계산하지 않으므로 **실브라우저 아니면 안 드러난다** —
  `test_every_css_variable_and_class_actually_exists` 가 `var(--x)` 전수와 app.js 의
  클래스를 CSS 와 대조한다. 런타임 주입(`--village-img`)과 의도적 무스타일(인라인
  style + 프로브 셀렉터 훅 7개)만 목록 예외이고, **목록이 낡으면 그것도 잡는다.**
...
- **상단 탭은 6개뿐이다(§7.9 — 2026-08-13 사용자 지시로 7개, §7.17 로 6개).** 마을·개요·
  ...
  없는 링크를 지어내지 말고, 무링크 카드는 **상세 오버레이**로 연다(2026-08-24 사용자
  지시 "다른 카드들도 전부 다" — `build_overview` 가 `hist` 를 싣고 `app.js`
  `openOvDetail` 이 이력 차트·변화 6구간·표본 메타를 그린다. hist 는 무링크 카드에만
  싣는다 — 링크 카드에 실으면 이중 게시, e2e 가 강제). 눌러도 무동작인 카드는 여전히
  금지다. `SECTION_LABELS` 는 각 섹션 `<h2>` 와 **문자 단위로 같아야 한다**
...
- **§7.13 산문 접기 — 화면 기본은 답·경고만, 설명은 「ⓘ 설명」 클릭 뒤다**
  (2026-08-21 사용자 지시 "불필요한 글자가 너무 많아 — 심플하게"). 정본은 `app.js`
  `explainBox(id, [{label}], ...children)` + `style.css` `details.explain`/`.explain-body`,
  index.html 의 정적 안내문 10곳은 같은 마크업을 손으로 쓴다(재렌더가 없어 네이티브 토글).
  판정 규칙: **경고·사유·관측일·구속 ⚠·폴백 이유·헤드라인 숫자는 절대 접지 말 것** —
  접힌 정보는 없는 정보라, 접는 순간 §7.7.17·§7.8.1·「조용한 대체 금지」가 무력화된다.
  접는 것은 방법론 해설·산식 풀이·사용법·규약 배경뿐이다. 구현 계약 셋:
  ① 토글은 **코드가 직접** 연다(preventDefault + `open` 속성 반전 — 셰이드에 네이티브
  토글이 없어 프로브가 실행으로 재려면 이 길뿐이다) ② 열림 상태는 `EXPLAIN_OPEN`
  (세션 메모리)이 id 로 유지한다 — recalc 가 DOM 을 다시 만들 때 열어 둔 설명이 닫히지
  않게. localStorage 저장은 하지 않는다(새로 열면 접힌 기본값이 의도) ③ **테스트가
  참조하는 문자열을 `title` 속성으로 옮기지 말 것** — textContent 에서 사라진다. 접힌
  details 는 textContent 에 남으므로 기존 문자열 검사는 그대로 성립하고, 「보이는가」는
  프로브의 `visText()`(닫힌 explain 본문 제외)로 잰다. 프로브 `explainFold` 19측정이
  기본 닫힘·토글·재렌더 내성·⚠ 가시성을 실행으로 고정한다. **새 설명 산문을 추가할 때는
  기본 노출이 아니라 explainBox 로 넣는 것이 기본값이다.**
