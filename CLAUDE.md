...
# 테스트 (합성 픽스처 — ../Data 없이 돈다, 약 4분). 현재 359개.
...
- **자본시장가정(CMA) = `bm.py` 가 정본이다** (§7.7 재설계의 데이터층 — `alloc.json.cma`).
  ...
  폐형 재구성. `_alt` 가 실리면 `_fx` 는 K+2 번째다 — 위치가 아니라 이름으로 집을 것). **월말 표본**으로만 계산한다 — 주말 캐리포워드 일별을 그대로 쓰면 σ 가
...
- **자산배분 화면의 첫 자리는 시뮬레이터다** (2026-08-11 사용자 지시 — 2026-08-05
  ...
  **헤지 슬라이더에도 최적 ▼ 마커가 붙는다**(§7.7.15) — 비중 막대와 같은 `.sim-bar-wrap`
  ...
  버튼을 눌러도 값이 그대로인 경우의 **사유를 카드가 적는다**(비중 0 슬리브 · 밴드 물림 ·
  이미 대표점) — 무반응을 고장으로 읽지 않게 하는 자리다.
