...
- **마을 안내판 = `paintVillageNotes`·`villageNotesModel` 이 정본이다(§7.18, 2026-09-07
  ...
  **상자는 4.6줄 높이 고정이고 글은 줄 단위로 위로 흐른다**(2026-09-07 2차 지시 "4~5줄,
  ...
  (여관 +7 — 배너와 겹침 방지).
- **리본 배너 = `villageBannerSvg`·`mountVillageBanner`·`VILLAGE_BANNER`(§7.19, 2026-09-07).**
  지도 좌상단 하늘 x 5%·y 3%·폭 19% — 겹침을 피해 고른 값(리본 오른쪽 끝 ≈22.5% < 여관
  안내판 24.3%). **파일이 없다** — 코드가 SVG 한 장을 그린다. 사용자 제공 영상을 키잉해
  올렸던 첫 판(e660b01)은 "마을은 귀여운데 배너는 너무 심각하다"는 톤 불일치 지적으로 같은
  날 걷어냈고, 계약 테스트가 `assets/village-banner.*` 부재와 자산 참조 부재를 고정한다.
  글꼴은 SVG 에 지정하지 않는다(body 스택 상속 = 마을 라벨과 같은 글꼴 — 테스트가
  `font-family` 부재를 본다). 색은 지도에서 딴 값, ✉ 는 관문 엠블럼 글리프. 흔들림은
  CSS(`.vb-sway`)이고 reduced-motion 이면 CSS 가 세운다 — **JS 분기 없음**(정적 SVG 는
  그 자체로 무모션이라 마운트를 가를 이유가 없다). 클릭 통과. 셰이드는 innerHTML 을
  파싱하지 않으므로 프로브 `villageBanner` 는 마크업 문자열을 본다(`villageFxMarkup` 과
  같은 방식). 사연과 되돌리기 금지 사유는 `dashboard/assets/README.md`.
