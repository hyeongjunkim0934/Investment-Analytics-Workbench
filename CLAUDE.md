...
- **마을 안내판 = `paintVillageNotes`·`villageNotesModel` 이 정본이다(§7.18, 2026-09-07
  ...
  종탑=위험·국면, 여관=이벤트 브리핑, 곳간=참고치−현재 배분, 교역소=헤지·Xe. 붙는 건물과
  위/아래는 `VILLAGE_NOTES` 가 정한다(교역소만 아래 — 종탑과 세로로 가까워서).
  **새 숫자가 없다** — 점수·등급은 risk.json, 브리핑은 원고 첫 줄+3문장 원문, 배분·헤지
  차이는 **`allocRefModel` 한 벌**(자산배분 요약표 `#alloc-summary` 와 같은 함수 — 프로브
  `villageNotes` 가 문자 단위 일치를 고정한다. 따로 계산하지 말 것). 참고 표시 전용이라
  동사(늘림/줄임/매수/매도) 금지 — 정규식으로 잡는다. 기준은 마지막 **저장값**(없으면
  「기본값·미저장」이라 적는다). 페이로드가 없으면 그 상자만 사유를 적는다.
