...
| `pipeline/common.py` | 공용 산식 한 벌 — `epoch_seconds`/`pack_values`/`spearman`/`auc`. 위 셋과 연구 하네스가 전부 여기서 가져온다 |
...
- **JSON 13개 계약은 세 곳에 적혀 있다**: `process.py` 의 `payloads`, `dashboard/app.js` 의 `FILES`,
...
- **테스트는 합성 픽스처로만 돈다.** `tests/synth.py` 가 만드는 워크북에는 벤더 값이 한 톨도 없다 —
  ...
  파이프라인을 완주시켜 시리즈 수·JSON 13개·경고 건수를 변경 전과 비교하는 방식으로 한다.
