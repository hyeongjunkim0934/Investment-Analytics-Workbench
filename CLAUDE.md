...
- **리스크 → 최적화 통합 프로세스 카드 = `renderAllocRiskProc` 가 정본이다**
  (`#alloc-risk-proc`, 자산배분 탭 맨 아래 — §7.16, 2026-09-01 사용자 지시). 재료는
  `risk.json` `layers.*.hist_m`(월말 점수 전 구간 — `risk.py` 가 게시, e2e 계약 있음),
  계산은 브라우저: 점수 → λ(**앵커 = 사용자의 화면 λ**, 점수 50 ↔ 화면 λ — 권장값
  없음 §7.7.12. 로그 = 등급 한 칸당 ×10 기본 / 선형 = 점수/50 토글) → 매월
  `optimizeUtilAt`(반복 `RP_ITERS`=1200 — 3000 대비 0.006%p 실측) → 누적 100% 스택
  SVG. **참고 표시 전용** — 등급·경보·λ 키인에 자동 반영하지 않는다(HANDOVER §5.1 ⓑ
  미채택 유지 — 이 카드는 표시이지 연동이 아니다). μ·Σ·밴드 현재 설정 고정(백테스트
  아님 — 화면이 적는다). CMA 층 전용·hist_m 부재 시 보류 사유 명시. 층·매핑 상태는
  `rp_layer`/`rp_map`(관측 설정 — 즉시 저장). 프로브
  `allocRiskProc` 13측정이 합계 100·σ*(λ) 단조·참고 표시 계약·토글·보류 사유를
  실행으로 확인한다.
