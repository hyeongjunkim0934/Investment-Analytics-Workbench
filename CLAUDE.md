...
- **자산배분 화면의 위험 원천은 스위치다 — CMA(기관 벤치마크)가 기본, 벤더 프록시는 대조용.**
  `app.js` `allocEngine` 의 `layer` 분기가 정본: `CMA_ACCT_LBL`(회계 8키→벤치마크 라벨),
  `buildCma`(로딩·환노출·잔차), `allocCmaSrcTag`(출처 태그). CMA 비활성이면 **사유를 적으며**
  프록시로 물러난다(`layerNote` — 조용한 대체 금지). 핵심 성질 셋, 전부 프로브 `cmaLayer` 가
  실행으로 확인한다: ① BM 통계는 **환노출 제거 기준**(실측: corr(시가 해외채권, 달러원) −0.59)
  이라 미헤지분 (1−h) 만큼 `_fx` 를 **더한다** — Xe 붕괴는 CMA 층에서도 정확히 성립.
  ② **대체투자 위험 = 기관 방식 팩터 매핑을 분류별로**(2026-08-12 지분형/대출형 분리 —
  회계 축은 9키·경제 축은 7키가 됐고 두 회계 키는 벤치마크 계열 하나(시가 대체투자)를
  공유한다). 기본 매핑은 **두 분류 모두 해외주식·국내채권 50/50**(2026-08-12 사용자
  지시 "전에 말한대로 5대5" — 기관 현행 방식). 분류별 상이 매핑(예: 지분형 65/35·
  대출형 0/100)은 콘솔에서 민감도 대안으로 조정한다.
  ...
  ③ **회계 관점 최적화는 CMA 층에서만 돈다**(§7.7 ① — 프록시층은 장부가 σ=0 이라 §7.2-1
...
- **자산배분 화면의 첫 자리는 시뮬레이터다** (2026-08-11 사용자 지시 — 2026-08-05
  ...
  정본: 9자산군 — **순서도 계약이다**(2026-08-12 사용자 지정: 장부가 국내채권 →
  장부가 해외채권 → 단기자금 → 국내주식 → 해외주식 → 시가 국내채권 → 시가 해외채권 →
  대체투자(대출형) → 대체투자(지분형) = `ALLOC_ACCT`). **대출금은 배분 우주에서
  제외**(2026-08-12) — 9개가 합 100 이고 구 「준고정 12%」 칸·목표 88 은 폐지됐다.
  **μ 디폴트 = 사용자 지정 CMA 수치**(`DEFAULTS.mu_over`/`by_kr`/`by_fx` — 2026-08-12.
  ...
  **조용히 덮지 않고** 화면 표시(`keyed-off-default`)와 「μ·σ 디폴트로 되돌리기」
  ...
  구 배분 8칸은 이 패널이 같은 id(`sim-mix-*`)로 승계했다. 그 아래는 종전대로
  「요약 → 조작(설정) → 설명」 순이다:
- **자산배분 화면의 나머지 동선은 「요약(답) → 조작(콘솔) → 설명」 순이다** (2026-08-05 사용자
  확인 — 담당자의 메인 용례는 ① 최적 배분·헤지 조회 ② 자유 조정 시뮬레이션 둘이다).
  `#alloc-summary` 가 현재 vs 참고치(배분 6 + 헤지 채/주 + Xe + 수익·위험·갭)를 한 표로
  답하고, 콘솔의 배분 8칸·헤지 슬라이더 2개는 **즉시 반영·저장 안 함**이다 — 저장은
