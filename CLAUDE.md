...
# 테스트 (합성 픽스처 — ../Data 없이 돈다, 약 2분). 현재 344개.
...
- **자산배분 화면의 위험 원천은 스위치다 — CMA(기관 벤치마크)가 기본, 벤더 프록시는 대조용.**
  ...
  공유한다). 기본 매핑은 지분형 = 해외주식 65/국내채권 35, 대출형 = 0/100 (콘솔 조정).
...
- **자산배분 화면의 첫 자리는 시뮬레이터다** (2026-08-11 사용자 지시 — 2026-08-05
  ...
  정본: 9자산군(대체투자 지분형/대출형) μ·σ 키인(σ 는 `st.sig_over` — **키인 σ ×
  벤치마크 실측 ρ**, 상관은 건드리지 않는다. 엔진의 `sigScale`/`scaleCmaM` 이 D·M·D 로
  적용하고 앵커는 관측 σ 유지. 대체투자 두 분류는 σ 키인 비활성 — 매핑이 정함),
  λ-MVO 최적(막대 위 ▼)과 비중 막대·숫자 즉시 시뮬레이션. **도넛 두 개는 각자
  자기 카드의 열(`sim8-col`) 아래 중앙**이다 — ① 최적 카드 밑 최적 도넛, ② 시뮬 카드
  밑 시뮬 도넛, 크기 210(2026-08-12 사용자 지시 — 카드와 위치 일치).
