...
# 테스트 (합성 픽스처 — ../Data 없이 돈다, 약 4분). 현재 400개.
...
- **추정일 시나리오(§7.10) = `app.js` 의 `estScenario`, 산식 정본은 `estimate.SCENARIO_MODEL`.**
  ...
  **`a.kpi-link` 에 `display` 를 주지 말 것** — `.kpi` 의 flex+gap 이 죽어 카드 내부 간격이
  사라진다(회귀 테스트 있음).
- **기준일·추정일은 한 표에 나란히다(§7.11 — 2026-08-13 사용자 지시).** 표는 하나이고
  헤더가 **2행**이다: `자산군 ‖ 기준일(규모·비중·듀레이션·수익률·출처·운용수익) ‖
  추정일(캐리·가격효과·환효과·스왑MTM·수익률·차이) ‖ 메모`. 폐지된 세 열은 **연환산
  계수·반영 수익률·기여도**이고(기여도는 바로 아래 막대그래프가 같은 값을 보여 준다),
  되살리면 계약 테스트가 잡는다. 경계선은 `.est-sep` 클래스인데 **재계산이 className 을
  통째로 다시 쓰므로 `put()` 안에서 같이 붙여야 한다** — 빠뜨리면 한 번의 재계산에
  조용히 사라진다. **4항 분해는 통합 표에만 그릴 것** — 결과 카드(`#est-scenario-result`)
  에도 그리면 같은 수가 한 화면에 두 번 나오고 한쪽만 고쳤을 때 갈린다.
