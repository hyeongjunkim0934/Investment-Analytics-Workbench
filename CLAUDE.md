...
- **두루마리 배너 = `mountVillageBanner`·`villageBannerSvg`·`VILLAGE_BANNER`(§7.19, 2026-09-07).**
  사용자 제공 수채 일러스트(편지 문 올빼미 + 두루마리)를 흰 배경만 키잉한 `assets/village-banner.webp`
  를 지도 좌상단 하늘 x 5%·y 2.5%·폭 19% 에 얹는다(오른쪽 끝 24% < 여관 안내판 24.3% — 실측).
  **제목은 그림에 굽지 않는다** — `villageBannerSvg()` 가 두루마리 면 위에 SVG 텍스트로 얹고,
  글꼴을 지정하지 않아 마을 라벨과 같은 body 스택을 상속한다. 좌표 `VILLAGE_BANNER.text`
  (면 중심·기울기 8°)와 `vb`(키잉본 캔버스 721×568)는 **한 몸**이라 그림을 갈면 함께 고쳐야
  한다(종횡비가 다르면 제목이 두루마리를 벗어난다). 흔들림은 CSS(`.vb-sway` — 그림과 제목을
  한 래퍼로 묶어 같이 움직인다)이고 reduced-motion 이면 CSS 가 세운다 — **JS 분기 없음**
  (정지 그림은 그 자체로 무모션). 클릭 통과. 셰이드는 innerHTML 을 파싱하지 않으므로 프로브
