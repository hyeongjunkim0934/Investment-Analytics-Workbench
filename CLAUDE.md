...
- **리스크연계 = `renderAllocRiskProc`**, 별도 workspace의 `현재 / 잠재` 탭이다.
  ...
  점수→λ 매핑은 사용자 화면 λ를 앵커로 유지한다(로그 기본, 선형 선택). μ·Σ·밴드는 현재
  기관 설정에 고정한 시나리오 경로이며 역사적 백테스트가 아니다. 등급·경보·사용자 λ 자동
  변경 없음. 화면 하단 날짜/λ 판독문·산식·설명은 2026-09-10 요청으로 제거했다.
  `rp_layer`·`rp_map`·`rp_range`는 관측 설정으로 저장하고 이 카드만 다시 그린다.
  ...
  제공한다. CSV는 선택 기간의 원시 정밀도를 유지한다. CMA 부재·잘못된 이력·제약 불충족은
