...
# 테스트 (합성 픽스처 — ../Data 없이 돈다, 약 2분). 현재 340개.
...
- **자산배분 화면의 위험 원천은 스위치다 — CMA(기관 벤치마크)가 기본, 벤더 프록시는 대조용.**
  ...
  ② **대체투자 위험 = 기관 방식 팩터 매핑**(시가 해외주식·시가 국내채권, 기본 50/50 —
  2026-08-11 사용자 확인)이고 잔차(고유위험)를 `_alt` 회귀로 폐형 가산한다 — 잔차 없이
  넣으면 공분산이 특이행렬이다(실측 최소고유값 −1.8e−18). 상태는 `st.alt_map`.
...
- **자산배분 화면의 첫 자리는 시뮬레이터다** (2026-08-11 사용자 지시 — 2026-08-05
  ...
  정본: 8자산군 μ·σ 키인(σ 는 `st.sig_over` — **키인 σ × 벤치마크 실측 ρ**, 상관은
  건드리지 않는다. 엔진의 `sigScale`/`scaleCmaM` 이 D·M·D 로 적용하고 앵커는 관측 σ
  유지), λ-MVO 최적(막대 위 ▼ + 도넛)과 비중 막대·숫자 즉시 시뮬레이션(카드 + 도넛).
