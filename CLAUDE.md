...
- **15는 코드가 정한 수**(아래 JSON 계약)라 달라지면 그 자체가 버그다. 반대로 `N`·`M`·경고 건수는
  ...
  **495 시리즈 / 7 파일 / 경고 13건**(duplicate column 11건 = 진짜 중복 skip 1건(`미국_IRS_5y1y`)
  ...
  하나로 합쳐지고 12M 선행 EPS·지수 8열이 붙음 — §7.21) 이전 체크아웃이면 483 / 8 / 경고 14건,
...
- 같은 시트에 같은 Notation이 두 번 나오면 **첫 컬럼이 그 라벨을 갖고**, 뒤 컬럼은 Notation 행
  **아래**의 헤더 행(블룸버그 티커·필드 행)에서 첫 컬럼과 다른 셀을 접미사로 붙여 `라벨 (접미사)`
  키로 살린다(`process.dup_suffix`, 2026-09-09 §7.21 — 예: `bb:KOSPI` = 12M 선행 EPS,
...
- **밸류에이션·이익 리비전(잠재 위험)은 12M 선행 EPS 로 산다(§7.21, 2026-09-09 업로드).**
