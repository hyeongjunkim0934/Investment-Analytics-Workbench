...
# 테스트 (합성 픽스처 — ../Data 없이 돈다, 약 4분). 현재 421개.
...
- **자산배분 화면의 위험 원천은 스위치다 — CMA(기관 벤치마크)가 기본, 벤더 프록시는 대조용.**
  ...
  **총 환노출 ≠ Xe** — 매핑 대체투자가 시가 해외주식(h₀=0)을 팩터로 써서 `_fx` 열이
  0 인데도 슬리브당 `we/100` 만큼 환을 진다. 헤지 레버가 닿지 않는 몫이라 Xe 에 넣지
  않고, 엔진 `fxLoadW()` 가 내재분까지 세어 화면이 **Xe 와 구분해서** 적는다(실측
  대표 배분: Xe 6.00% 외에 8.00% — 표기의 2.33배가 실제).
