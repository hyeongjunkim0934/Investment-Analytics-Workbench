...
# 테스트 (합성 픽스처 — ../Data 없이 돈다, 약 4분). 현재 361개.
...
- **자산배분 화면의 첫 자리는 시뮬레이터다** (2026-08-11 사용자 지시 — 2026-08-05
  ...
  **헤지 슬라이더에도 최적 ▼ 마커가 붙는다**(§7.7.15) — 비중 막대와 같은 `.sim-bar-wrap`
  ...
  않게 하는 자리다. **구속의 출처를 뭉치지 말 것**(재점검 발견): `bandBinds`(내규 밴드가
  구간을 좁힘)와 `capBinds`(**해외자산 비중 자체**가 환노출 상한 — 밴드를 풀어도 못 간다)는
  다른 문장을 쓴다. 중립 밴드인데 「밴드가 물고 있다」라고 적으면 사용자를 밴드 완화라는
  **틀린 조치**로 보낸다(실측: 디폴트 λ 에서 xeFree 15.59% > xeHi 15.00% = 총 해외비중).
  판정은 xe 가 아니라 **xeFree 가 [xeLo,xeHi] 밖인지**로 하고 허용오차는 상대값이다 —
  수렴 잔차가 제약 판정으로 새면 아무것도 안 무는 상태에서 경고가 뜬다(실측 λ=100).
