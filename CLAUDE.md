...
- **16은 코드가 정한 수**(아래 JSON 계약)라 달라지면 그 자체가 버그다. 반대로 `N`·`M`·경고 건수는
  ...
  **483 시리즈 / 8 파일 / 경고 14건**(duplicate column 12건 = `data_bb (3).xlsx` 5건 +
  ...
  표본이 10년을 채우면 저절로 사라진다)이며, 이 기준선의 정본은 Data 저장소
  쪽(`../Data/CLAUDE.md` — 그쪽 문서의 13건은 port 이전 수치라 +1 갱신 필요)이다.
  2026-08-21 교체 전 체크아웃(구 `data_bb.xlsx`)이면
  481 / 8 / 경고 16건으로 나온다. 급감이나 경고 성격 변화만 신호로 볼 것.
...
- **자산배분 화면의 물리적 최상단은 §7.14 포트폴리오 구성 패널이고(2026-08-21 사용자
  ...
  **통화별 환헤지 분해**(`allocCcyHedgeRows` + 접이식 표)는 슬리브 헤지비율을 통화
  ...
  「요약 → 조작(설정) → 참고치·레버」까지다(설명 구역 — 특성·자산군 표·투자선·이행
...
- **수익률 추정(§7.8) = 화면이 계산하고 파이프라인은 지수만 싣는다.** 산식 정본은
  ...
  ① **입력값이 그 자체로 연환산 수익률이다**(주식 제외 — §7.11, 같은 날 2차 지시로
  §7.8 의 「화면이 계수를 곱한다」를 대체). 담당자가 기관 시스템에서 이미 연환산해
  들고 오므로 **화면이 다시 곱하면 이중 연환산**이다(실측 대비: 1.50% 입력이 3.02% 로
  나가던 상태). 정본은 `estimate.ANNUALIZE["input_is_annualized"]`.
  계수(365 ÷ 경과일수, 연초 = **전년 12/31**, 1/1 이 아니다)는 없어지지 않았다 —
  **추정일 시나리오에서만** 쓴다(기간수익 되돌리기 + 추정일 재연환산). 6/30 이면
  181일 → 2.0166 이고, 월수 기준이면 정확히 2.0 이 되므로 프로브가 「2.0 이 아님」을
  함께 고정한다(규약이 조용히 되돌아가는 것을 막는 자리다).
  ② **주식(국내·해외)만 연환산하지 않는다** — 나머지는 수익이 안정적으로 확보된다는 가정.
  ③ **듀레이션은 추정일 가격효과(−D×Δy)에 쓰인다** — 기준일 수익률 계산에는 안 쓰이고,
  **화면이 그 구분을 밝힌다**. §7.8 당시의 「지금 계산에 쓰이지 않습니다」 문장은
  §7.10 이 실제로 쓰기 시작하면서 **거짓이 됐다** — 되살리지 말 것(회귀 테스트 있음).
...
- **추정일 시나리오(§7.10) = `app.js` 의 `estScenario`, 산식 정본은 `estimate.SCENARIO_MODEL`.**
  `추정 기간수익률 = 캐리 + 가격효과 + 환효과 + 스왑 MTM`. **부호 둘이 이 화면의 심장이다** —
  가격효과 = **−**D×Δy(금리 상승 = 가격 하락), 스왑 MTM = h×τ×(**−**Δ스왑). 후자는
  `hedge.py` 회계모형 ⑤ 와 같은 부호이며 **새 모형을 만들지 말 것**(그 모형이 정본이다).
  자산군→축 매핑은 `EST_SCEN`: **장부가는 `rate` 없음**(원가법이라 가격효과 0), 다만
  **장부가 해외채권은 `fx:true`** 라 환·스왑으로 움직인다(2026-08-13 사용자 설명 — 채권은
  원가법이어도 환헤지 스왑은 파생이라 MTM 이 난다). 시장 축은 `estimate.json.axes` 6개이고
  `kind` 가 변화량의 뜻을 정한다(rate=차이, price=변화율) — 화면이 스스로 정하게 두면
  금리를 비율로 나누는 사고가 난다. `estScenario(A, st)` 는 base 를 **스스로 만든다**:
  밖에서 받으면 다른 `st` 로 만든 base 가 들어와 듀레이션이 어긋난 채 조용히 계산된다.
  **입력 칸을 되쓸 때는 포커스를 확인할 것(§7.10.1 CRITICAL)** — `<input type=number>` 는
  `-` 하나만 친 순간 value 가 "" 라, 되쓰면 사용자가 친 문자가 지워지고 자동값 뒤에 붙는다
  (실측 `-3.5` → `20.0035`, 저장까지 됐다). **뜻 없는 헤드라인을 만들지 말 것** —
  수익률이 하나도 없으면 `Σ수익 ÷ Σ규모` 가 0 을 주는데 0% 는 「계산했더니 0」이라는
  뜻이라 「아직 못 냈다」와 다르다(`portBlockedNoRet`).
  **`a.kpi-link` 에 `display` 를 주지 말 것** — `.kpi` 의 flex+gap 이 죽어 카드 내부 간격이
  사라진다(회귀 테스트 있음).
- **기준일·추정일은 한 표에 나란히다(§7.11 → §7.12 — 2026-08-13 사용자 지시).** 표는
  하나이고 헤더가 **2행**이다: `자산군 ‖ 기준일(규모·비중·듀레이션·수익률) ‖
  추정일(규모·비중·수익률·차이) ‖ 근거`. 폐지된 열은 **연환산 계수·반영 수익률·기여도**
  (§7.11)와 **출처·4항 분해**(§7.12)이고, 되살리면 계약 테스트가 잡는다 — 기여도는 아래
  막대그래프가, 4항은 결과 카드 접이식이, 출처는 `#est-sources` 접이식이 이어받았다.
  경계선은 `.est-sep` 클래스인데 **재계산이 className 을 통째로 다시 쓰므로 `put()` 안에서
  같이 붙여야 한다** — 빠뜨리면 한 번의 재계산에 조용히 사라진다.
  **4항 분해는 결과 카드에만 그릴 것** — 표에도 그리면 같은 수가 한 화면에 두 번 나오고
  한쪽만 고쳤을 때 갈린다.
  **이미 `<td>` 인 셀을 다시 `<td>` 로 감싸지 말 것** — 브라우저가 중첩을 풀며 셀이 하나 더
  생겨 그 행만 열이 밀린다(실측: calc 행 11칸 / carry 행 10칸). **DOM 셰이드는 중첩을
  그대로 두어 통과시키므로 프로브가 못 잡는다** — 실브라우저가 잡은 자리다.
- **추정일 쪽 사용자 입력은 규모뿐이다(§7.12).** 비우면 기준일 규모를 승계하고 승계값을
  **칸에 실제로 표시**한다(상태에는 넣지 않는다 — 넣으면 기준일 규모를 바꿔도 옛 값이
  눌러앉는다). 수익률은 `EST_SCEN[].mode` 가 정한다: `calc` 7개(장부가 해외채권·국내주식·
  해외주식·시가 국내/해외채권 직접·간접)는 **수기 칸을 만들지 않고**, `carry` 4개
  (장부가 국내채권·단기자금·대출금·대체투자)는 승계 + 수기 덮어쓰기다. 가르는 선은
  「가격 축이 있는가」이고 **사용자가 자산군을 하나씩 지목해 정했다** — 임의로 옮기지 말 것.
  사유는 파이프라인 `estimate.ROW_MODES` 가 싣고 화면이 「근거」 열에 적는다(두 파일이
  갈리면 `tests/test_estimate.py` 가 잡는다).
  **가중치가 두 벌이다** — 기준일 열은 기준일 규모로, 추정일 열은 **추정일 규모**로 가중한다.
  한 벌로 재면 리밸런싱 효과가 통째로 사라진다.
- **시장지표 카드는 수준을 나란히 보여 주고 사용자는 추정일 수준을 친다(§7.12).**
  변화(bp/%)는 `estAxisLevels` 가 계산한다 — 수준↔변화 환산을 사용자에게 시키면 금리는 bp,
  환율은 % 라 단위를 오갈 때마다 실수가 난다. 표시 단위는 파이프라인의 `level_unit`/
  `level_dp` 가 정한다(화면이 스스로 정하면 달러원을 % 로 적는다). 구 저장분(`dlt`)은
  `estMigrateLevels` 가 **기준일 수준 + Δ** 로 1회 이관하고, 가드는 **`saved.dlt`** 로
  본다 — `st.dlt` 는 기본값으로 항상 존재해 그걸로 가드하면 매 렌더마다 다시 돈다.
- **헤지비율은 자산군을 눌러 0~105% 에서 고른다(§7.12) — 기본값을 박지 말 것.**
  기관의 현재 정책(해외채권 100% 헤지·해외주식 100% 오픈)은 운용 정보라 공개 저장소에
  넣지 않는다(2026-08-13 사용자 확인). 고르기 전에는 환효과·스왑 MTM 을 **0 으로 지어내지
  않고** 「헤지비율 미입력」이라고 적는다 — 0 으로 채우면 미헤지 자산의 환위험이 사라진 수가
  보고 숫자로 들어간다. 범위 정본은 `estimate.HEDGE_BAND`(화면 폴백 `EST_HEDGE_BAND`)이고
  105% 는 오버헤지 상한이라 그 구간에서 **환효과 부호가 뒤집힌다**. 밴드 밖 값은 조용히
  받지 않고 되돌린다. 여는 함수는 `openEstHedge()`.
- **묵은 자동값 경고(§7.8.1)를 「출처」와 함께 내리지 말 것.** 그것은 출처 정보가 아니라
  「지금 보는 수가 며칠 전 값이다」라는 **값에 대한 경고**다 — §7.12 에서 출처 열을 내리며
  실제로 함께 사라졌고 프로브가 잡았다. 지금은 「근거」 열에 붙는다.
...
- **경고색은 결과가 비었을 때만 쓸 것.** `priceNote` 에는 「장부가 — 원가법이라 가격효과
  없음」처럼 **정상 동작을 설명하는 문장**도 들어 있어서, 문구가 있다고 빨갛게 칠하면 계산이
  잘 된 행이 고장난 것처럼 읽힌다(실측). 판정은 `cumAnnual == null` 로 한다.
  두 열을 잇는 산식은 `estimate.SCENARIO_MODEL["cumulative"]` 가 정본이다:
  기준일 값이 연환산이므로 **사용자 규칙의 정확한 역**으로 기간수익까지 되돌린 뒤
  (× 경과일수 ÷ 365) 4항을 더하고 추정일 기준으로 재연환산한다. 캐리가 `r × days/365`
  라 대수적으로 `추정일 = 기준일 + (가격+환+스왑) × 365 ÷ 연초→추정일 일수` 가 되고,
  **두 열의 차이가 곧 시장효과의 연환산분**이다 — 되돌리기를 빠뜨리고 `r + total` 로
  더하면 연율과 기간이 섞인다(프로브가 이 항등식을 고정한다).
  **추정일이 다른 해면 누적을 잇지 말 것**(`crossYear`) — 그 해 1/1 → 기준일 구간의
  수익을 모른다. 옛 코드는 그때도 수를 만들고 추정일 연도의 짧은 경과일수로 연환산했다
  (2026-07-21 → 2027-03-31 이면 ×365/90 = 4.06배). 추정 구간은 그대로 낸다.
  **재계산은 `recalcAll()` 한 벌이다** — `E`/`S` 를 한 번씩 만들어 표·요약·기여도·축·
  결과카드 다섯 곳을 그린다. 따로 돌리면 같은 행의 왼쪽과 오른쪽이 다른 상태로 그려진다.
  포트폴리오 「차이」는 행별 가중합이 아니라 **두 헤드라인의 뺄셈**이다(추정 불가 행이
  기준일 쪽에만 들어가 세 수가 서로 안 맞는 것을 막는다 — 원인은 화면이 따로 적는다).
  DOM 순서도 바뀌었다 — 시나리오 입력 카드가 **표보다 위**다(표의 오른쪽 절반을 만드는
  입력이므로). 문서 전체 셀렉터로 `.est-table input`[0] 을 집으면 이제 축 입력이 잡힌다.
