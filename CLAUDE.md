...
- **리스크연계 = `renderAllocRiskProc`**, 별도 workspace의 `현재 / 잠재` 탭이다.
  ...
  2026-09-12 요청에 따라 원점수(0~100)를 그대로 λ로 사용한다. 기존 로그/선형 변환과
  기관 λ 앵커는 리스크연계에 사용하지 않는다. `portModelInputs`를 공유해 현재 포트폴리오
  6자산의 μ·σ·상관 입력과 선택 표본을 읽는다. μ·Σ 고정 시나리오이며 역사적 백테스트가 아니다.
  일반 효율적 경계선과 같은 공매도 금지·합계 100% 조건이다. 그룹 입력은 현재 비중 초기화용이며
  최적화 제약이 아니다. 리스크 등급·경보·기관 저장 λ는 변경하지 않는다. 화면 하단 날짜/λ 판독문·산식·설명은 2026-09-10 요청으로 제거했다.
  `rp_layer`·`rp_range`만 관측 설정으로 저장하고 이 카드만 다시 그린다. 기존 `rp_map`은 무시한다.
  관측 사이 비중은 직전 해를 유지하는 계단형 누적 면적이며 점수=λ 및 호버 λ를 표시한다.
