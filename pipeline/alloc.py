...
- 평균-분산 최적화 (Markowitz 1952) + 밴드 제약, 투영 경사법(역행렬 불사용).
  자산군 우주는 시가 기준 7축 하나 — 장부가 축은 2026-08-12 배분 우주에서
  제외했다(원가법 BM 의 σ 는 시장위험을 나르지 않는다 — §7.7.11).
...
DEFAULTS = {
    # 자산군 우주는 **시가 기준 7축 하나**다(2026-08-12 사용자 지시 — 장부가 축 제거
    # §7.7.11: 원가법 BM 의 손익변동 σ 는 시장위험을 나르지 않아 MVO 대상이 아니다.
    # 구 mix_acct/bands_acct/by_kr/by_fx 디폴트는 축과 함께 폐지 — app.js allocState 가
    # 구 저장분을 채권 쌍 합산으로 접는다). 대체투자는 지분형/대출형 두 분류(2026-08-12).
    # **대출금도 배분 우주에서 제외** — 7개 자산군이 합 100 을 만든다.
    # 예시 분할·비중은 자리표시자다(공개 저장소 — 실제 북 구성 아님).
    ...
    # 대체투자 환헤지 비율 % — 2026-08-19 사용자 지시("유동적으로 환헤지 중"). 위 둘과
    # **성격이 다르다**: 해외채권·해외주식 헤지는 최적화가 고르는 레버(시뮬레이션 트랙)인
    # 반면 이쪽은 **모형 입력**이라 즉시 저장된다(사용자가 고른 규약 — μ·σ·λ 와 같은 칸).
    # 값은 위 둘과 같은 자리표시자이며 기관 수치가 아니다(공개 저장소 — 실제 값은
    # 브라우저 localStorage 에만 남는다). CMA 층 전용이다: 프록시층의 대체투자 기저는
    # 원화(한국 CPI)라 환노출이 구조적으로 0 이고 이 비율이 걸릴 자리가 없다.
    "h_alt": 90,
...
def build(series_store: dict, warn) -> dict:
    ...
    return {
        ...
        "boot": {"note": ("기준 상태(defaults·예시 입력)로 빌드 시점 사전계산 — 수기 입력을 "
                 ...
                 "rows": boot},
        "checks": checks,
        # 회계 손익 5항 모형 — 장부가 축 폐지(§7.7.11)로 자산배분 화면은 더 이상
        # 렌더하지 않지만, JSON 계약(REQUIRED_KEYS) 안정성과 환헤지 화면의 동일
        # 모형 참조를 위해 게시는 유지한다.
        "acct_model": [
            "① 유효이자 — 상수 (북일드, 수기 입력)",
            "② 환산손익 = 장부금액 × Δ환율",
        ...
        "limits": ("기대수익 디폴트는 사용자 지정 CMA 수치 [키인 정본]입니다 — 키인은 헤지비용 반영 후 "
                   ...
                   "대체투자 α·위험은 수기 입력 [가정]입니다. 환율 기대변동은 0으로 "
