...
DEFAULTS = {
    # 대체투자는 지분형/대출형 두 분류로 나뉜다(2026-08-12 사용자 지시). 예시 분할은
    # 12/3 — 특정 기관의 실제 북 구성이 아니라 자리표시자다(공개 저장소).
    "mix": {"국내채권": 42.0, "해외채권": 18.0, "국내주식": 3.0, "해외주식": 5.0,
            "대체투자(지분형)": 12.0, "대체투자(대출형)": 3.0, "단기자금": 5.0},
...
def mu_vector(covS, rates, d, cost):
    ...
    return np.array([
        ...
        rates["cpi"]["v"] + d["alt_alpha"],      # 대체투자(지분형) — CPI+α [가정]
        rates["cpi"]["v"] + d["alt_alpha"],      # 대체투자(대출형) — 같은 자리표시자(키인으로 대체)
        rates["kr3m"]["v"],
    ]), a, sig
