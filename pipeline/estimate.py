...
**⚠ 이 게시물을 읽는 화면은 더 이상 없다.** 수익률 추정 화면은 §7.17(2026-09-07
사용자 지시)로 삭제됐고, JSON 계약 16 을 흔들지 않으려고 파이프라인만 남겼다
(HANDOVER §7.17 — 파이프라인까지 걷어낼지는 사용자 결정 대기). 아래 설명의 「화면」은
그 시점까지의 소비자를 가리킨다.

**이 모듈은 수익률을 계산하지 않았다.** 계산(연환산·기여도·포트폴리오 합산)은 전부
사용자 입력에 의존해 브라우저가 했다(`dashboard/app.js` 의 `estEngine` — 함께 삭제됨).
...
# 자동 채움 대상 — `asset` 은 삭제된 화면의 `EST_ASSETS` 이름과 짝이었다(§7.17 로
# 대조 상대가 사라져 교차검증 테스트도 함께 내렸다. 화면을 되살리면 그 대조부터 복구할 것).
INDICES = [
    {
        "key": "kospi_tr",
        "asset": "국내주식",
