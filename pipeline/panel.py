...
VARS = [
    ...
    # 환헤지 비용(스왑레이트) — 사용자 관심 종속변수(2026-08-12 §7.7.14). 위험수준
    # (risk 블록의 stress/vuln)과의 상관·교차상관·OLS 를 이 화면에서 바로 볼 수 있게
    # 화이트리스트에 올린다. 값 공개는 새 문제가 아니다 — 같은 계열(월말)이 이미
    # hedge.json cost_hist_usd 로 게시되고 있다. 부호 규약 동일: 양수 = 받음.
    ("fx_swap3m", "환헤지 비용 (3개월 스왑레이트)", "FX", "%", "rate", "info:SMB_USDKRW_3M"),
